---
skill: databases/notion
when: "Connecting to Notion databases, querying Notion pages"
requires: []
---
...
| Don't | Do |
|-------|-----|
| Put `type: "notion"` in fetch_data | Define type in `settings/database.json` |
| Use database ID from URL with dashes | Copy exact ID from Notion share/API |
| Store API key in code | Use `[[env.NOTION_API_KEY]]` |
| Forget schema for filters | Define schema for proper type handling |
| Query without schema | Framework can't infer types without schema |
| Expect instant updates | Notion API has slight delay for changes |
| Use `query` field at root | Use `filters` for consistency |
| Mix simple and native formats | Choose one format per query |
| Forget `findOne` for single records | Use `findOne: true` or `single: true` |
| Hardcode page IDs for relations | Query by property values when possible |
...
- `skill:database` - General database operations
- `skill:component` - Using databases in components
